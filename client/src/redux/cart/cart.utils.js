...
export const subtractItemFromCart = (cartItems, cartItemToSubtract) => {
  const existingCardItem = cartItems.find(cartItem => cartItem.id === cartItemToSubtract.id);
  ...
  return cartItems.map(cartItem =>
    cartItem.id === cartItemToSubtract.id ? { ...cartItem, quantity: cartItem.quantity - 1 } : cartItem,
  );
};
